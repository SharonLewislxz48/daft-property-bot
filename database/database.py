...
class EnhancedDatabase:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            if user:
                ...
                if not settings:
                    ...
                    await self.create_default_settings(user_id, db)
    ...
    async def create_default_settings(self, user_id: int, db: aiosqlite.Connection):
        """Создает настройки по умолчанию для пользователя"""
        await db.execute("""
            INSERT INTO user_settings (user_id, regions, min_bedrooms, max_price, monitoring_interval, max_results_per_search, is_monitoring_active) 
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (user_id, '["dublin-city"]', 3, 2500, 3600, 50, 0))
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            if row:
                return {
                    ...
                    "min_bedrooms": row[2],
                    "max_price": row[3],
                    "monitoring_interval": row[4],
                    "max_results_per_search": row[5],
                    "is_monitoring_active": bool(row[6]),
                    "created_at": row[7],
                    "updated_at": row[8]
                }
            return None
    ...
    async def clear_user_regions(self, user_id: int) -> bool:
        """Очищает все регионы пользователя и устанавливает dublin-city по умолчанию"""
        return await self.update_user_settings(user_id, regions=["dublin-city"])
    ...
    async def get_user_properties_count(self, user_id: int) -> int:
        """Получает общее количество найденных объявлений для пользователя"""
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute(
                "SELECT COUNT(*) FROM property_history WHERE user_id = ?", 
                (user_id,)
            ) as cursor:
