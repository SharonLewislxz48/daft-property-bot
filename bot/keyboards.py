...
def get_regions_menu_keyboard() -> InlineKeyboardMarkup:
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton(text="🔄 Заменить все", callback_data="replace_all_regions"),
        ],
        [
            InlineKeyboardButton(text="🏠 Главное меню", callback_data="main_menu")
        ]
...
def get_category_regions_keyboard(category: str, page: int = 0, callback_prefix: str = "select_region_") -> InlineKeyboardMarkup:
        ...
        for j in range(2):
            if i + j < len(page_regions):
                ...
                row.append(InlineKeyboardButton(
                    text=display_name,
                    callback_data=f"{callback_prefix}{region_key}"
                ))
        keyboard.append(row)
...
def get_popular_combinations_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=keyboard)
    """Клавиатура для выбора региона с пагинацией"""
    regions_list = list(ALL_LOCATIONS.items())
    total_pages = math.ceil(len(regions_list) / page_size)
    
    start_idx = page * page_size
    end_idx = min(start_idx + page_size, len(regions_list))
    
    keyboard = []
    
    # Добавляем регионы
    for i in range(start_idx, end_idx):
        region_key, region_name = regions_list[i]
        keyboard.append([
    ...
    nav_row = []
    if page > 0:
        nav_row.append(InlineKeyboardButton(text="⬅️", callback_data=f"region_page_{page-1}"))
    
    nav_row.append(InlineKeyboardButton(text=f"{page+1}/{total_pages}", callback_data="current_page"))
...
def get_replace_regions_keyboard() -> InlineKeyboardMarkup:
    """Клавиатура для режима замены регионов"""
    keyboard = [
        [
            InlineKeyboardButton(text="🏙️ Районы Дублина (62)", callback_data="replace_category_dublin_areas"),
            InlineKeyboardButton(text="🌆 Основные города (6)", callback_data="replace_category_main_cities")
        ],
        [
            InlineKeyboardButton(text="🗺️ Графства Ирландии (26)", callback_data="replace_category_republic_counties"),
            InlineKeyboardButton(text="🏴󠁧󠁢󠁳󠁣󠁴󠁿 Сев. Ирландия (6)", callback_data="replace_category_northern_counties")
        ],
        [
            InlineKeyboardButton(text="⭐ Популярные комбинации", callback_data="replace_category_popular"),
        ],
        [
            InlineKeyboardButton(text="🔙 Назад", callback_data="manage_regions"),
            InlineKeyboardButton(text="🏠 Главное меню", callback_data="main_menu")
